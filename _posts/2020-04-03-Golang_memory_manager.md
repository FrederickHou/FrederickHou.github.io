...
- **object：** 由span按特定大小切分的小块内存，每一个可存储一个对象；
    ...
    object，按8字节倍数分为n种。如，大小为24的object可存储范围在17~24字节的对象。在造成一些内存浪费的同时减少了小块内存的规格，优化了分配和复用的管理策略，分配器还会将多个微小对象组合到一个object块内，以节约内存。
...
Running the escape analysis command with go tool compile "-m" test.go will confirm the allocation made by Go:
...
可以看到在运行到smallAllocation函数时调用了runtime.newobject函数。此函数是Go的内存分配内置函数。源码如下：
...
根据PMG调度模型大家应该知道每个线程M将分配一个处理器P,并且一次最多处理一个goroutine。在分配内存时，当前的goroutine将使用其当前P的本地缓存，在span列表中找出第一个空闲的对象。使用本地缓存不需要锁，这样可以使内存分配效率更高。

span 列表(mspan)分为70个大小级别。从8 bytes到32k bytes，这样就可以存储不同大小的对象。如下图所示：
...
现在，我们想象一下如果span 再分配的时候没有空前的内存，将发生什么？
Go维护
